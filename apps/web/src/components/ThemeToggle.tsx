...
export function ThemeToggle() {
  ...
  return (
    <div>
      <p className="mb-1.5 text-[12.5px] font-semibold text-ink-soft">{t("label")}</p>
      <div className="flex overflow-hidden rounded-control border border-line">
        {opts.map(({ value, icon }) => (
          <button
            ...
            onClick={() => pick(value)}
            className={cls(
              "flex flex-1 items-center justify-center gap-1.5 px-3 py-2 text-[13.5px] font-semibold transition-colors",
