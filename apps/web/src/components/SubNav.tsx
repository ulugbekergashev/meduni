 ...
 * bitta joy").
 */
export function SubNav({ title, items, activeKey, footer }: SubNavData) {
  ...
  return (
    <div className="mb-3 flex max-w-full snap-x snap-mandatory gap-1 overflow-x-auto rounded-control border border-line bg-surface p-1 shadow-card [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:hidden">
      {items.map((item) => (
        <Link
          key={item.key}
          to={item.to}
