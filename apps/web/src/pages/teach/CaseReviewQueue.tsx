  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <p className="truncate text-[13.5px] font-semibold text-ink">{item.studentName}</p>
        {item.status === "PENDING" ? (
  ...
  return (
    <Card className="p-0">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between gap-2 px-4 py-3 text-left">
        <span className="text-[13.5px] font-bold text-ink-soft">{t("caseBrief")}</span>
      ...
      {open && (
        <div className="space-y-3 border-t border-line px-4 py-3">
          {rows.map((r) => r.text && (
            <div key={r.label}>
              <p className="mb-0.5 flex items-center gap-1.5 text-[11.5px] font-bold uppercase tracking-wide text-ink-faint">
                <Icon icon={r.icon} size={13} /> {r.label}
              </p>
              <p className="whitespace-pre-line text-[13px] text-ink">{r.text}</p>
            </div>
          ))}
        </div>
      )}
...
function ReviewPanel({ id, onSavedNext, onClose }: { id: number; onSavedNext: () => void; onClose: () => void }) {
  ...
  if (detailQ.isError || !detail) return <Card><p className="py-6 text-center text-[13.5px] text-rose">{t("loadError")}</p></Card>;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2 lg:hidden">
        <button onClick={onClose} className="flex items-center gap-1 text-[13.5px] font-medium text-brand-deep">
      ...
      <div className="space-y-3">
        {detail.questions.map((q, i) => (
          <Card key={i} className="space-y-3">
            <p className="text-[14px] font-semibold text-ink">{i + 1}. {q}</p>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-control border border-line bg-bg p-3">
                <p className="mb-1 text-[11.5px] font-bold uppercase tracking-wide text-ink-faint">{t("studentAnswer")}</p>
                <p className="whitespace-pre-line text-[13.5px] text-ink">{detail.answers[i]}</p>
              </div>
              <div className="rounded-control bg-emerald-soft p-3">
                <p className="mb-1 text-[11.5px] font-bold uppercase tracking-wide text-emerald">{t("reference")}</p>
                <p className="whitespace-pre-line text-[13.5px] text-ink">{detail.referenceAnswer[i]}</p>
              </div>
            </div>
          </Card>
        ))}
      ...
      <Card className="space-y-3">
        <div className="flex items-center gap-3">
          <label className="text-[13.5px] font-semibold text-ink">{t("score")}</label>
          <input
            type="number"
            min={0}
            max={100}
            ...
            className="w-24 rounded-control border border-line px-3 py-2 text-[15px] font-bold outline-none focus:border-brand"
            placeholder="0–100"
          />
        ...
        <textarea
          ...
          rows={3}
          className="w-full rounded-control border border-line px-3 py-2 text-[13.5px] outline-none focus:border-brand"
        />
        <div className="flex flex-wrap gap-1.5">
          {templates.map((tpl) => (
            <button
              ...
              className="rounded-pill border border-line px-2.5 py-1 text-[12px] text-ink-soft transition-colors hover:bg-bg"
            >
              + {tpl}
            </button>
          ))}
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <select value={query.sort} onChange={(e) => patch({ sort: e.target.value as QueueQuery["sort"] })} className="rounded-control border border-line bg-surface px-2 py-2 text-[13px] outline-none focus:border-brand">
        ...
        <div className="relative min-w-[160px] flex-1">
          ...
          <input value={query.search} onChange={(e) => patch({ search: e.target.value })} placeholder={t("searchStudent")} className="w-full rounded-control border border-line bg-surface py-2 pl-9 pr-3 text-[13.5px] outline-none focus:border-brand" />
        </div>
      </div>
        ...
        <div className={cls(selected !== null && "hidden lg:block")}>
          ...
          {queue.length === 0 && !queueQ.isLoading && !queueQ.isError && (
            <Card className="flex flex-col items-center gap-2 border-emerald/40 bg-emerald-soft py-10 text-center">
              ...
              <p className="text-[14px] font-bold text-emerald">{t("emptyQueue")}</p>
            </Card>
          )}
        </div>
        ...
        <div className={cls(selected === null && "hidden lg:block")}>
          {selected !== null ? (
            ...
            <Card className="hidden lg:flex lg:min-h-[40vh] lg:items-center lg:justify-center">
              <p className="text-[13.5px] text-ink-faint">{t("selectPrompt")}</p>
            </Card>
          )}
        </div>
      </div>
