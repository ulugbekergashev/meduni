    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <input value={item} onChange={(e) => onChange(items.map((x, j) => (j === i ? e.target.value : x)))} placeholder={placeholder} className="flex-1 rounded-control border border-line px-3 py-2 text-note outline-none focus:border-brand" />
      ...
      <button onClick={() => onChange([...items, ""])} className="inline-flex items-center gap-1 text-note font-medium text-brand-deep hover:underline">
...
export function SyllabusTab() {
  ...
  if (q.isError || !q.data) return <Card><p className="py-6 text-center text-note text-rose">{t("loadError")}</p></Card>;

  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-note text-ink-soft">{t("subtitle")}</p>
        <span className="inline-flex items-center gap-1.5 rounded-pill bg-brand-soft px-3 py-1 text-note font-semibold text-brand-deep">
      ...
      <Card className="space-y-5">
        <div>
          <h3 className="mb-2 text-note font-bold text-ink-soft">{t("description")}</h3>
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder={t("descriptionPlaceholder")} rows={3} className="w-full rounded-control border border-line px-3 py-2 text-note outline-none focus:border-brand" />
        </div>
        <div className="grid gap-5 lg:grid-cols-2">
          <div>
            <h3 className="mb-2 flex items-center gap-1.5 text-note font-bold text-ink-soft"><Icon icon={Target} size={14} /> {t("objectives")}</h3>
          ...
          <div>
            <h3 className="mb-2 flex items-center gap-1.5 text-note font-bold text-ink-soft"><Icon icon={BookText} size={14} /> {t("literature")}</h3>
      ...
      <div>
        ...
        {topics.length === 0 ? (
          <Card><p className="py-6 text-center text-note text-ink-soft">{t("noTopics")}</p></Card>
        ) : (
          <div className="overflow-x-auto rounded-card border border-line">
            <table className="w-full border-collapse text-note">
              <thead>
                <tr className="border-b border-line bg-bg text-left text-note font-bold text-ink-faint">
                ...
                {topics.map((tp, i) => (
                  <tr key={tp.id} className="border-b border-line last:border-0">
                    ...
                    <td className="px-3 py-2">
                      <input type="number" min={0} value={tp.hours} onChange={(e) => setTopic(tp.id, { hours: Number(e.target.value) })} className="w-16 rounded-control border border-line px-2 py-1 text-note outline-none focus:border-brand" />
                    </td>
                    <td className="px-3 py-2">
                      <input value={tp.note} onChange={(e) => setTopic(tp.id, { note: e.target.value })} placeholder={t("notePlaceholder")} className="w-full rounded-control border border-line px-2 py-1 text-note outline-none focus:border-brand" />
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-note text-ink-faint">{t("note2")}</span>
      </div>
    </div>
  );
}
