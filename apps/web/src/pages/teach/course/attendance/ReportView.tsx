...
function Matrix({ report }: { report: AttReport }) {
  ...
  return (
    <div className="overflow-x-auto rounded-card border border-line">
      <table className="border-collapse text-note">
        <thead>
          <tr>
              ...
              {report.sessions.map((s) => {
                ...
                return (
                  <td key={s.id} className="border-b border-line p-0.5 text-center">
                    {cell ? (
                      <span
                        className={cls("inline-flex h-6 min-w-[26px] items-center justify-center rounded px-1 text-micro font-bold", STATUS_META[cell.status].solid)}
      ...
      {rows.map((s) => {
        ...
        return (
          <Card key={s.id} className="flex flex-wrap items-center gap-3">
            <span className="min-w-[130px] flex-1 text-body font-semibold text-ink">{s.fullName}</span>
            <div className="flex flex-wrap gap-2 text-note">
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        <input type="date" value={range.from} onChange={(e) => setRange((r) => ({ ...r, from: e.target.value }))} className="rounded-control border border-line px-2 py-2 text-note outline-none focus:border-brand" />
        <span className="text-ink-faint">—</span>
        <input type="date" value={range.to} onChange={(e) => setRange((r) => ({ ...r, to: e.target.value }))} className="rounded-control border border-line px-2 py-2 text-note outline-none focus:border-brand" />
        <div className="relative min-w-[150px] flex-1">
          <Icon icon={Search} size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ink-faint" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("searchStudent")} className="w-full rounded-control border border-line bg-surface py-2 pl-9 pr-3 text-note outline-none focus:border-brand" />
        </div>
        {view === "list" && (
          <select value={sort} onChange={(e) => setSort(e.target.value as "pct" | "name")} className="rounded-control border border-line bg-surface px-2 py-2 text-note outline-none focus:border-brand">
            <option value="pct">{t("sortPct")}</option>
            <option value="name">{t("sortName")}</option>
          </select>
        )}
        <div className="flex overflow-hidden rounded-control border border-line">
          <button onClick={() => setView("matrix")} className={cls("flex items-center gap-1 px-3 py-2 text-note font-medium", view === "matrix" ? "bg-brand-soft text-brand-deep" : "text-ink-soft hover:bg-bg")}><Icon icon={LayoutGrid} size={15} /> {t("matrix")}</button>
          <button onClick={() => setView("list")} className={cls("flex items-center gap-1 px-3 py-2 text-note font-medium", view === "list" ? "bg-brand-soft text-brand-deep" : "text-ink-soft hover:bg-bg")}><Icon icon={List} size={15} /> {t("summary")}</button>
        </div>
        <a href={exportUrl} className="inline-flex items-center gap-1.5 rounded-control border border-line px-3 py-2 text-note font-medium text-ink-soft transition-colors hover:bg-bg"><Icon icon={Download} size={15} /> Excel</a>
      </div>
