...
function CellModal({ courseId, target, onClose }: { courseId: number; target: CellTarget; onClose: () => void }) {
  ...
  return (
    <Modal open onClose={onClose} title={target.studentName} className="max-w-sm">
      <p className="mb-3 text-note text-ink-faint">{target.sessionLabel}</p>
        ...
        {STATUSES.map((s) => {
          ...
          return (
            <button
              key={s}
              onClick={() => setStatus(s)}
              className={cls("rounded-control border px-2 py-2 text-note font-semibold transition-all", on ? meta.solid : `bg-surface ${meta.ring} opacity-60 hover:opacity-100`)}
            >
              {t(`status.${s}`)}
            </button>
          );
      ...
      <div className="mt-4">
        <label className="mb-1 block text-note font-semibold text-ink-soft">{t("grade")}</label>
        <input
          type="number"
          min={0}
          max={100}
          ...
          placeholder="0–100"
          autoFocus
          className="w-full rounded-control border border-line px-3 py-2 text-body font-bold outline-none focus:border-brand"
        />
        <p className="mt-1 text-micro text-ink-faint">{t("gradeHint")}</p>
      </div>

      {err && <p className="mt-2 text-note font-medium text-rose">{err}</p>}
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <p className="min-w-[160px] flex-1 text-note text-ink-faint">{t("hint")}</p>
                            ...
                            return (
                              <td key={s.id} className="border-b border-line p-0.5 text-center">
                                <button
                                  ...
                                  className={cls(
                                    "inline-flex h-8 w-full min-w-[46px] items-center justify-center rounded text-note font-bold transition-transform hover:scale-110",
