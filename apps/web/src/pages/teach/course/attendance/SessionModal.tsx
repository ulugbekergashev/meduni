  ...
  return (
    <Modal open onClose={onClose} title={edit ? t("editSession") : t("newSession")}>
      <div className="space-y-3">
        <div>
          <label className="mb-1 block text-note font-semibold text-ink-soft">{t("date")} *</label>
          <input type="date" value={date} onChange={(e) => { setDate(e.target.value); setErr(null); }} className="w-full rounded-control border border-line px-3 py-2 text-note outline-none focus:border-brand" />
        </div>

        <div>
          <div className="mb-1 flex items-center gap-3 text-note font-semibold text-ink-soft">
          ...
          {mode === "text" ? (
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t("titlePlaceholder")} className="w-full rounded-control border border-line px-3 py-2 text-note outline-none focus:border-brand" />
          ) : (
            <select value={topicId} onChange={(e) => setTopicId(e.target.value ? Number(e.target.value) : "")} className="w-full rounded-control border border-line bg-surface px-3 py-2 text-note outline-none focus:border-brand">
        ...
        <div>
          <label className="mb-1 block text-note font-semibold text-ink-soft">{t("room")}</label>
          <input value={room} onChange={(e) => setRoom(e.target.value)} placeholder="302" className="w-full rounded-control border border-line px-3 py-2 text-note outline-none focus:border-brand" />
        </div>

        {err && <p className="text-note font-medium text-rose">{err}</p>}
