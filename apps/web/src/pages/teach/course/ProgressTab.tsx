        ...
        {topics.map((tp) => {
          ...
          return (
            <div key={tp.id} className="rounded-control border border-line p-3">
              ...
              <p className="mt-1 text-[12.5px] text-ink-soft">{cellSummary(c, t)}</p>
              {c.state !== "COMPLETED" && (
                <button
                  onClick={() => unlock.mutate({ studentId: student.id, topicId: tp.id }, { onSuccess: () => show(t("unlocked")) })}
                  disabled={unlock.isPending}
                  className="mt-2 inline-flex items-center gap-1 rounded-control border border-amber/40 bg-amber-soft px-2.5 py-1 text-[12px] font-semibold text-amber transition-colors hover:bg-amber/10"
                >
    ...
    <div className="overflow-x-auto rounded-card border border-line">
      <table className="border-collapse text-[12.5px]">
        ...
        <tbody>
                  ...
                  return (
                    <td key={tp.id} className="border-b border-line p-0.5">
                      <button
                        ...
                        className={cls("flex h-8 w-full items-center justify-center rounded text-[10px] font-bold transition-transform hover:scale-110", cellClass[c?.state ?? "LOCKED"])}
                      >
          ...
          {/* group average row */}
          <tr className="bg-bg">
            <td className="sticky left-0 z-10 border-r border-line bg-bg px-3 py-2 text-[11.5px] font-bold uppercase text-ink-faint">{t("groupAvg")}</td>
            {data.topics.map((tp) => (
              <td key={tp.id} className="px-1 py-2 text-center text-[11px] font-semibold text-ink-soft">{avgByTopic.get(tp.id)}%</td>
            ))}
            <td className="border-l border-line px-2 py-2 text-center text-[11px] font-bold text-ink-soft">{data.stats.avgProgress}%</td>
          </tr>
        </tbody>
      </table>
    </div>
      ...
      {students.map((s) => (
        <Card key={s.id} className={cls("flex flex-wrap items-center gap-4", s.behind && "border-rose/40")}>
          <div className="min-w-[140px] flex-1">
            <p className={cls("text-[14px] font-semibold", s.behind ? "text-rose" : "text-ink")}>{s.fullName}</p>
            <div className="mt-1 flex items-center gap-2">
              ...
              <span className="text-[12px] font-semibold text-ink-soft">{s.overallPct}%</span>
            </div>
          </div>
        ...
        >
          {data && (
            <>
              ...
              <div className="mt-5 flex flex-wrap items-center gap-2">
                <div className="relative flex-1 min-w-[180px]">
                  ...
                  <input
                    ...
                    className="w-full rounded-control border border-line bg-surface py-2 pl-9 pr-3 text-[13.5px] outline-none focus:border-brand"
                  />
                </div>
                <select value={sort} onChange={(e) => setSort(e.target.value as "progress" | "name")} className="rounded-control border border-line bg-surface px-2 py-2 text-[13px] outline-none focus:border-brand">
                  <option value="progress">{t("sortProgress")}</option>
                  <option value="name">{t("sortName")}</option>
                </select>
                <div className="flex overflow-hidden rounded-control border border-line">
                  <button onClick={() => setView("heatmap")} className={cls("flex items-center gap-1 px-3 py-2 text-[13px] font-medium", view === "heatmap" ? "bg-brand-soft text-brand-deep" : "text-ink-soft hover:bg-bg")}>
                    <Icon icon={LayoutGrid} size={15} /> {t("heatmap")}
                  </button>
                  <button onClick={() => setView("list")} className={cls("flex items-center gap-1 px-3 py-2 text-[13px] font-medium", view === "list" ? "bg-brand-soft text-brand-deep" : "text-ink-soft hover:bg-bg")}>
                    <Icon icon={List} size={15} /> {t("list")}
                  </button>
                </div>
                <a href={`${API_URL}/api/v1/teach/courses/${courseId}/progress/export?view=${view}`} className="inline-flex items-center gap-1.5 rounded-control border border-line px-3 py-2 text-[13px] font-medium text-ink-soft transition-colors hover:bg-bg">
              ...
              <div className="mt-4">
                {students.length === 0 ? (
                  <Card><p className="py-6 text-center text-[13.5px] text-ink-soft">{t("noMatch")}</p></Card>
                ) : view === "heatmap" ? (
