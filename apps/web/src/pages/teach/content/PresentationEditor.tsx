...
import { ArrowLeft, Download, FileDown, ImageOff, Plus, RefreshCw, Sparkles, Trash2 } from "lucide-react";
          ...
          <Card key={slide.id} className="grid gap-4 md:grid-cols-[1fr_18rem]">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-2 text-note font-bold text-ink-soft">
                  {si + 1}
            ...
            <div>
              {slide.imageSlots.length === 0 ? (
                <div className="flex aspect-video items-center justify-center rounded-control bg-bg text-note text-ink-faint">
                  {t("noImage")}
                </div>
              ) : (
                <>
                  <label className="mb-2 flex cursor-pointer items-center gap-2 text-note font-medium text-ink-soft">
                    <input
                      type="checkbox"
