      ...
      <div className="mt-5">
                ...
                <td className="px-4 py-3">
                  <Link to={`/admin/users/${u.id}`} className="group flex items-center gap-3">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block truncate text-note text-ink-faint">{u.email}</span>
                    </span>
                  </Link>
                </td>
                <td className="px-4 py-3">
                  <span className="block font-medium text-ink">{u.groupName ?? "—"}</span>
                  {u.facultyName && <span className="block text-micro text-ink-faint">{u.facultyName}</span>}
                </td>
                <td className="px-4 py-3">
                  {u.progressPct === null ? (
                    <span className="text-note text-ink-faint">—</span>
                  ) : (
        ...
        {data && items.length > 0 && (
          <div className="mt-4 flex items-center justify-between gap-3">
            <span className="text-note text-ink-faint">{t("totalCount", { count: data.total })}</span>
            {totalPages > 1 && (
