    ...
    <div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        <div className="relative min-w-[150px] flex-1">
          ...
          <input value={filters.actor} onChange={(e) => patch({ actor: e.target.value })} placeholder={t("actor")} className="w-full rounded-control border border-line bg-surface py-2 pl-9 pr-3 text-[13.5px] outline-none focus:border-brand" />
        </div>
        <select value={filters.action} onChange={(e) => patch({ action: e.target.value })} className="rounded-control border border-line bg-surface px-2 py-2 text-[13px] outline-none focus:border-brand">
          <option value="">{t("allActions")}</option>
          {(data?.actions ?? []).map((a) => <option key={a} value={a}>{a}</option>)}
        </select>
        <input type="date" value={filters.from} onChange={(e) => patch({ from: e.target.value })} className="rounded-control border border-line px-2 py-2 text-[13px] outline-none focus:border-brand" />
        <span className="text-ink-faint">—</span>
        <input type="date" value={filters.to} onChange={(e) => patch({ to: e.target.value })} className="rounded-control border border-line px-2 py-2 text-[13px] outline-none focus:border-brand" />
      </div>
          ...
          <AsyncSection isLoading={false} isError={q.isError} isEmpty={!!data && data.items.length === 0} emptyIcon={<Icon icon={ScrollText} size={22} />} emptyText={t("empty")} onRetry={() => q.refetch()}>
                ...
                <tbody>
                  {data?.items.map((it) => (
                    <tr key={it.id} className="border-b border-line last:border-0 hover:bg-bg">
                      ...
                      <td className="px-3 py-2.5"><span className="font-medium text-ink">{it.actorName}</span> <span className="text-[11px] text-ink-faint">{it.actorRole}</span></td>
                      ...
                      <td className="px-3 py-2.5 text-right">{it.details ? <button onClick={() => setDetail(it)} className="text-[12.5px] font-medium text-brand-deep hover:underline">{t("view")}</button> : <span className="text-ink-faint">—</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {data && data.pages > 1 && (
              <div className="mt-3 flex items-center justify-center gap-3">
                ...
                <span className="text-[13px] text-ink-soft">{filters.page} / {data.pages}</span>
      ...
      <Modal open={detail !== null} onClose={() => setDetail(null)} title={detail?.action}>
        <pre className="max-h-[50vh] overflow-auto rounded-control bg-bg p-3 text-[12px] text-ink">{JSON.stringify(detail?.details, null, 2)}</pre>
      </Modal>
    </div>
  );
}
