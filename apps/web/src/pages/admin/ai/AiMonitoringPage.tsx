...
function QuotaBar({ pct }: { pct: number | null }) {
  if (pct === null) return <span className="text-micro text-ink-faint">—</span>;
    ...
    <Modal open onClose={onClose} title={t("editQuota")}>
      ...
      <div className="space-y-3">
        {[{ l: t("tokenLimit"), v: tok, s: setTok }, { l: t("imageLimit"), v: img, s: setImg }, { l: t("costLimit"), v: cost, s: setCost }].map((f) => (
          <div key={f.l}>
            <label className="mb-1 block text-note font-semibold text-ink-soft">{f.l}</label>
            <input type="number" min={0} value={f.v} onChange={(e) => f.s(e.target.value)} className="w-full rounded-control border border-line bg-surface px-3.5 py-2.5 text-body outline-none focus:border-brand" />
          </div>
        ))}
      </div>
...
function HeroMetric({ icon, value, label }: { icon: typeof Coins; value: string; label: string }) {
  return (
    <div className="flex items-center gap-3">
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-control bg-surface-raised text-ink-soft"><Icon icon={icon} size={20} /></div>
      <div>
        <p className="text-stat font-bold leading-none tabular-nums">{value}</p>
        <p className="mt-1 text-note font-medium text-ink-faint">{label}</p>
      </div>
    </div>
  );
}
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <input
          ...
          className="rounded-control border border-line bg-surface px-3.5 py-2.5 text-body text-ink outline-none focus:border-brand"
        />
      </div>
      ...
      {usage.isLoading ? (
        ...
        <>
          ...
          <div className="mt-5 rounded-card border border-line bg-surface p-4">
            <div className="flex items-center gap-2"><Icon icon={Sparkles} size={16} className="text-ink-soft" /><p className="text-note font-medium text-ink-faint">{t("thisMonth")}</p></div>
