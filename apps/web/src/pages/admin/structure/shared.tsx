...
export function CountChip({ children }: { children: ReactNode }) {
  return <span className="rounded-pill bg-bg px-2 py-0.5 text-micro font-medium text-ink-soft">{children}</span>;
}
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        {withAdmin && (
          <div className="rounded-control border border-line bg-bg/50 p-4">
            <p className="text-note font-bold text-ink">{adminSectionTitle}</p>
            <p className="mt-0.5 text-note text-ink-faint">{t("adminSectionHint")}</p>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              ...
              <Field label={t("adminPassword")}>
                ...
                <p className="mt-1 text-micro text-ink-faint">{t("adminPasswordHint")}</p>
              </Field>
            </div>
          </div>
        )}
        ...
        {withQuota && (
          <div className="rounded-control border border-line bg-bg/50 p-4">
            <p className="text-note font-bold text-ink">{t("quotaSection")}</p>
            <p className="mt-0.5 text-note text-ink-faint">{t("quotaSectionHint")}</p>
        ...
        {error && <p className="text-note text-rose">{error}</p>}
    ...
    <Modal open onClose={() => onClose()} title={t(`appoint.${role}`)}>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <Field label={t("fields.password")}>
          ...
          <p className="mt-1 text-note text-ink-faint">{t("fields.passwordHint")}</p>
        </Field>
        {error && <p className="text-note text-rose">{error}</p>}
        <div className="flex justify-end gap-2">
          <Button type="button" variant="ghost" onClick={() => onClose()}>{tc("cancel")}</Button>
  ...
  if (!admin) {
    return (
      <Card className="flex flex-wrap items-center gap-3 border-dashed !p-4">
        ...
        <p className="min-w-0 flex-1 text-note text-ink-soft">{t("noAdmin", { role: roleLabel })}</p>
        {canManage && (
  ...
  return (
    <Card className="flex flex-wrap items-center gap-4 !p-4">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-micro font-bold text-ink-faint">{roleLabel}</p>
        <p className="truncate text-body font-bold text-ink">{admin.fullName}</p>
        <p className="mt-0.5 flex flex-wrap gap-x-4 gap-y-0.5 text-note text-ink-soft">
      ...
      {canManage && (
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Link to={`/admin/users/${admin.id}`} className="text-note font-semibold text-brand-deep hover:underline">
            {t("profile")} →
          </Link>
        </div>
      )}
