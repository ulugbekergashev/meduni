...
export function ChipSelect({
  ...
  if (options.length === 0 && emptyText) {
    return <p className="text-[13px] text-amber">{emptyText}</p>;
  }
      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={cls(
              "inline-flex items-center gap-1 rounded-pill border px-3 py-1.5 text-[13px] font-medium transition-colors",
              isSel
