...
// from the design tokens (var(--...)) so they follow light/dark theme.
  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-[20px] font-bold leading-none tabular-nums text-ink">{Math.round(v)}%</span>
        {label && <span className="mt-0.5 text-[10.5px] text-ink-faint">{label}</span>}
      </div>
    </div>
  );
}
...
export function ProgressBar({ value, tone = "brand", className = "" }: { value: number; tone?: ToneKey; className?: string }) {
  const v = Math.max(0, Math.min(100, value));
  return (
    <div className={`h-2 w-full overflow-hidden rounded-pill bg-bg ${className}`}>
...
export function BarRow({
  ...
  return (
    <button
      onClick={onClick}
      disabled={!onClick}
      className={`flex w-full items-center gap-3 rounded-control px-1 py-1.5 text-left ${onClick ? "hover:bg-bg" : "cursor-default"}`}
    >
      <span className="w-40 shrink-0 truncate text-[13px] font-medium text-ink">{label}</span>
      ...
      <span className="w-10 shrink-0 text-right text-[12.5px] font-bold tabular-nums text-ink-soft">{Math.round(v)}%</span>
    </button>
  );
}
...
export function StackedBar({ segments }: { segments: { value: number; tone: ToneKey }[] }) {
  ...
  return (
    <div className="flex h-2.5 w-full overflow-hidden rounded-pill bg-bg">
      {segments.map((s, i) =>
        s.value > 0 ? <span key={i} style={{ width: `${(s.value / total) * 100}%`, background: toneVar[s.tone] }} /> : null
      )}
    </div>
  );
}
