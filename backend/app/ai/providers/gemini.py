"""Google Gemini через REST (free tier, ADR-0002). Без SDK — один httpx-вызов."""
import base64
import json
import time

import httpx

from app.ai.base import ImageModel, TextModel
from app.core.config import settings

_API = "https://generativelanguage.googleapis.com/v1beta/models"


class GeminiTextModel(TextModel):
    def __init__(self) -> None:
        if not settings.gemini_api_key:
            raise RuntimeError("GEMINI_API_KEY не задан в backend/.env")
        self.model = settings.gemini_text_model

    def generate_json(self, system: str, user: str, schema: dict) -> tuple[dict, int]:
        body = {
            "systemInstruction": {"parts": [{"text": system}]},
            "contents": [{"role": "user", "parts": [{"text": user}]}],
            "generationConfig": {
                "responseMimeType": "application/json",
                "responseSchema": schema,
                "temperature": 0.3,
            },
        }
        last_error: Exception | None = None
        for attempt in range(3):
            try:
                response = httpx.post(
                    f"{_API}/{self.model}:generateContent",
                    headers={"x-goog-api-key": settings.gemini_api_key},
                    json=body,
                    timeout=180,
                )
                if response.status_code in (429, 500, 503):
                    # free tier: лимиты запросов — ждём и повторяем
                    last_error = RuntimeError(f"Gemini HTTP {response.status_code}: {response.text[:300]}")
                    time.sleep(5 * (attempt + 1))
                    continue
                response.raise_for_status()
                payload = response.json()
                text = payload["candidates"][0]["content"]["parts"][0]["text"]
                tokens = payload.get("usageMetadata", {}).get("totalTokenCount", 0)
                return json.loads(text), tokens
            except (httpx.HTTPError, KeyError, json.JSONDecodeError) as e:
                last_error = e
                time.sleep(3 * (attempt + 1))
        raise RuntimeError(f"Gemini недоступен после 3 попыток: {last_error}")


class GeminiImageModel(ImageModel):
    """Генерация изображений Gemini (free tier). Модель — gemini_image_model в конфиге."""

    def __init__(self) -> None:
        if not settings.gemini_api_key:
            raise RuntimeError("GEMINI_API_KEY не задан в backend/.env")
        self.model = settings.gemini_image_model

    def generate_image(self, prompt: str) -> bytes:
        full_prompt = (
            "Educational medical illustration for a university lecture slide. "
            "Clean, clear, textbook style, no text or labels on the image. " + prompt
        )
        body = {
            "contents": [{"role": "user", "parts": [{"text": full_prompt}]}],
            "generationConfig": {"responseModalities": ["IMAGE"]},
        }
        last_error: Exception | None = None
        for attempt in range(3):
            try:
                response = httpx.post(
                    f"{_API}/{self.model}:generateContent",
                    headers={"x-goog-api-key": settings.gemini_api_key},
                    json=body,
                    timeout=180,
                )
                if response.status_code in (429, 500, 503):
                    last_error = RuntimeError(f"Gemini image HTTP {response.status_code}: {response.text[:200]}")
                    time.sleep(5 * (attempt + 1))
                    continue
                response.raise_for_status()
                payload = response.json()
                for part in payload["candidates"][0]["content"]["parts"]:
                    inline = part.get("inlineData") or part.get("inline_data")
                    if inline and inline.get("data"):
                        return base64.b64decode(inline["data"])
                raise RuntimeError("Gemini не вернул изображение")
            except (httpx.HTTPError, KeyError, ValueError) as e:
                last_error = e
                time.sleep(3 * (attempt + 1))
        raise RuntimeError(f"Gemini image недоступен после 3 попыток: {last_error}")
