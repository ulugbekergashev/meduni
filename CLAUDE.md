...
- **M7 (Посещаемость и геймификация) — готов.** Модуль attendance: LessonSession
  + динамический QR (TOTP-like, ротация 15с, `qr.py`), чек-ин студента (BarcodeDetector
  в браузере + ручной ввод), live-список, ручные правки с AuditLog, журнал XLSX,
  автоопределение опоздания. Модуль gamification: XPEvent/StudentStats/Badge,
  движок начисления XP (идемпотентно, `service.py`), стрики, бейджи (авто-выдача),
  лидерборды (группа/курс/факультет, неделя/семестр — топ-10 + своя позиция),
  XP-конфиг в админке. XP-хуки в learning/attendance. Экраны: `/teach/sessions/{id}`
  (большой QR), `/app/attendance`, `/app/rating`, `/app/profile`, `/admin/xp`.
  37 тестов. QR-занятие + рейтинг недели работают.
- **M8 (Аналитика и полировка) — готов.** Модуль analytics: расход AI по кафедрам
  за месяц (агрегация из GenerationJob.tokens_used), квоты (AIQuota + `check_quota`
  во всех generate-эндпоинтах — блокирует при исчерпании), статистика вуза,
  heatmap прогресса (студенты × темы), задачи преподавателя, просмотр AuditLog.
  Экраны: `/admin` (дашборд: KPI + AI usage + квоты), `/admin/audit`, задачи и
  heatmap на страницах преподавателя. 42 теста.

**Все 8 milestones (M1–M8) готовы — платформа функционально полная.**
Остаётся инфраструктурное (не в dev-скоупе): Sentry, бэкапы, прогон на PostgreSQL,
Celery/Redis на сервере, нагрузочный тест, пилот на кафедрах.
